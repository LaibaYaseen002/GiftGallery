...
import { createDailyRoom, getDailyRoom, deleteDailyRoom } from "../services/daily";
...
// POST /api/social-shopping/sessions/:code/video-room - Create or get video room (Auth, host only)
router.post("/sessions/:code/video-room", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = getUserId(req)!;
    const { code } = req.params;
    ...
    const { data: session, error: sessionError } = await supabase
      .from("shopping_sessions")
      .select("id, host_id, is_active, session_code, video_room_url, video_room_name")
      .eq("session_code", code)
      .single();
    ...
    if (session.host_id !== userId) {
      res.status(403).json({ error: "Only the host can start a video call" });
      return;
    }
    ...
    if (session.video_room_url && session.video_room_name) {
      const existingRoom = await getDailyRoom(session.video_room_name);
      if (existingRoom) {
        res.json({ data: { url: session.video_room_url, name: session.video_room_name } });
        return;
      }
    }
    ...
    res.status(201).json({ data: { url: room.url, name: room.name }, message: "Video room created" });
  } catch (err) {
...
// GET /api/social-shopping/sessions/:code/video-room - Get video room info (Auth)
router.get("/sessions/:code/video-room", requireAuth, async (req: Request, res: Response) => {
  try {
    const { code } = req.params;

    const { data: session, error: sessionError } = await supabase
      .from("shopping_sessions")
      .select("video_room_url, video_room_name, is_active")
      .eq("session_code", code)
      .single();
    ...
    res.json({
      data: {
        url: session.video_room_url,
        name: session.video_room_name,
        is_active: session.is_active,
      },
    });
  } catch (err) {
    console.error("Error fetching video room:", err);
    res.status(500).json({ error: "Failed to fetch video room info" });
  }
});

// DELETE /api/social-shopping/sessions/:code/video-room - End video call (Auth, host only)
router.delete("/sessions/:code/video-room", requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = getUserId(req)!;
    const { code } = req.params;
    ...
    const { data: session, error: sessionError } = await supabase
      .from("shopping_sessions")
      .select("id, host_id, video_room_name")
      .eq("session_code", code)
      .single();
    ...
    if (session.host_id !== userId) {
      res.status(403).json({ error: "Only the host can end the video call" });
      return;
    }

    // Delete room from Daily
    if (session.video_room_name) {
      await deleteDailyRoom(session.video_room_name);
    }

    // Clear room info from database
    await supabase
      .from("shopping_sessions")
      .update({ video_room_url: null, video_room_name: null })
      .eq("id", session.id);
